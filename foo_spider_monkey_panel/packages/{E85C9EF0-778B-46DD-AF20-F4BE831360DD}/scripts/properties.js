...
class PanelPropertiesUpdate {
	...
	update(properties) {
		...
		let nm = '';
		for (let i = 1; i < 100; i++) {
			prop = i == 1 ? ` View by Folder Structure: Name // Pattern` : ` View ${$.padNumber(i - 1, 2)}: Name // Pattern`;
			nm = this.getOrig(prop);
			if (nm && nm != ' // ') this.setNew(`View ${$.padNumber(i, 2)}: Name // Pattern`, nm);
			this.setOrigNull(prop);
		}

		for (let i = 1; i < 100; i++) {
			prop = ` View Filter ${$.padNumber(i, 2)}: Name // Query`;
			nm = this.getOrig(prop);
			if (nm && nm != ' // ') {
				this.setNew(`Filter ${$.padNumber(i, 2)}: Name // Query`, i == 1 ? `Filter // Button Name` : nm);
			}
			this.setOrigNull(prop);
		}
